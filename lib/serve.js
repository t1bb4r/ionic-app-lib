...
Serve.loadSettings = function loadSettings(argv, project) {
  ...
  if (argv.cert && argv.key) {
    ...
    options.cert = cert
    options.key = key
    options.livereloadOptions = options.livereloadOptions ? options.livereloadOptions : {}
    options.livereloadOptions.cert = cert
    options.livereloadOptions.key = key
  }
...
Serve.printCommandTips = function() {
  ...
  log.info('  restart'.cyan + ' or ' + 'r'.cyan + ' to restart the client app from the root');
  log.info('  goto'.cyan + ' or ' + 'g'.cyan + ' and a url to have the app navigate to the given url');
  log.info('  consolelogs'.cyan + ' or ' + 'c'.cyan + ' to enable/disable console log output');
  log.info('  serverlogs'.cyan + ' or ' + 's'.cyan + ' to enable/disable server log output');
  log.info('  quit'.cyan + ' or ' + 'q'.cyan + ' to shutdown the server and exit');
  log.info('');
};
...
Serve.startServer = function startServer(options, app) {
  ...
  // Listen
  try {
    console.log(options.cert, options.key, options.port, options.address);
    runningServer = https.createServer({cert: options.cert, key: options.key}, app).listen(options.port, options.address, function(){console.log('callback', arguments)});
  } catch (ex) {
    Utils.fail('Failed to start the Ionic server: ', ex.message);
  }

  log.info('√ Running dev server: ', options.devServer.cyan);
};
...
Serve.host = function host(address, port) {
  ...
  var hostAddress = ['https://', address, ':', port].join('');
  return hostAddress;
};
